#!/usr/bin/env bash
# Full CLI setup for the YouTube Research Agent using the Anthropic CLI (ant).
#
# Prerequisites:
#   brew install anthropics/tap/ant
#   export ANTHROPIC_API_KEY="sk-..."
#
# Run: bash setup_youtube_agent.sh

set -euo pipefail

echo "==> Creating YouTube Research Agent..."

AGENT_ID=$(ant beta:agents create \
  --name 'YouTube Research Agent' \
  --model '{"id": "claude-opus-4-8"}' \
  --system 'You are the YouTube Research Agent, a specialist in YouTube content strategy who helps creators plan high-performing videos. You operate in a chat UI and converse directly with the user.
...
Guardrails: Never invent real YouTube analytics, specific video URLs, or creator metrics. Clearly label all competitive insights as pattern-based reasoning, not live data. If the niche is too broad, ask to narrow down. Always ask about target audience, channel size, and content style before finalizing. Present deliverables in clearly labeled sections. If the user asks for revisions, iterate on the specific section only.

Tone: Confident, actionable, and concise. Speak like a seasoned YouTube strategist — no generic marketing fluff.' \
...
echo ""
echo "==> Setup complete. Export these for session use:"
echo ""
echo "  export YOUTUBE_AGENT_ID=$AGENT_ID"
echo "  export YOUTUBE_ENVIRONMENT_ID=$ENV_ID"
echo ""
echo "Then run a session:"
echo "  python run_youtube_session.py"
